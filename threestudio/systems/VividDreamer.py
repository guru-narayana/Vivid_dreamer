...
@threestudio.register("vividdreamer-system")
class VividDreamer(BaseLift3DSystem):
    @dataclass
    class Config(BaseLift3DSystem.Config):
        ...
        shift_guidance: bool = False
        use_multi_gaussian: bool = False
        num_gausssians: int = 1
    ...
    def configure(self) -> None:
        ...
        self.use_multi_gaussian = self.cfg.use_multi_gaussian
        self.num_gausssians = self.cfg.num_gausssians


        self.gaussian = GaussianModel(sh_degree = self.sh_degree)
        if self.use_multi_gaussian:
            self.gaussians = []
            for i in range(self.num_gausssians-1):
                self.gaussians.append(GaussianModel(sh_degree = self.sh_degree))
    ...
    def forward(self, batch: Dict[str, Any],renderbackground = None) -> Dict[str, Any]:
        ...
        for id in range(batch['c2w_3dgs'].shape[0]):
            ...
            if not self.use_multi_gaussian:
                render_pkg = render(viewpoint_cam, self.gaussian, self.pipe, renderbackground)
            else:
                id = torch.randint(0, len(self.gaussians), (1,)).item()
                render_pkg = render(viewpoint_cam, self.gaussians[id], self.pipe, renderbackground)
    ...
    def shit_guidance(self):
        del self.guidance
        self.cfg.guidance_type = "stable-diffusion-guidance"
        self.guidance = threestudio.find(self.cfg.guidance_type)(self.cfg.guidance)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_multi_gaussian:
            for g in self.gaussians:
                g.update_learning_rate(self.true_global_step)
        
        if self.true_global_step > 1200:
        ...
        if self.true_global_step == 800 and self.cfg.shift_guidance:
            print(f"Deleting {self.cfg.guidance_type},", end = "")
            self.shit_guidance()
            print(f" Now using {self.cfg.guidance_type}")
